...
def sample_feature_noise(X_size, E_size, y_size, node_mask):
    ...
    epsy = sample_gaussian(y_size) if y_size is not None else None

    float_mask = node_mask.float()
    epsX = epsX.type_as(float_mask)
    epsE = epsE.type_as(float_mask)
    epsy = epsy.type_as(float_mask) if y_size is not None else None
...
def sample_normal(mu_X, mu_E, mu_y, sigma, node_mask):
    ...
    mu_y_size = mu_y.size() if mu_y is not None else None
    eps = sample_feature_noise(mu_X.size(), mu_E.size(), mu_y_size, node_mask).type_as(mu_X)
    X = mu_X + sigma * eps.X
    E = mu_E + sigma.unsqueeze(1) * eps.E
    y = mu_y + sigma.squeeze(1) * eps.y if mu_y is not None else None
...
def sample_discrete_feature_noise(limit_dist, node_mask):
    ...
    y_limit = limit_dist.y[None, :].expand(bs, -1) if limit_dist.y is not None else None
    ...
    U_y = U_y.type_as(long_mask) if U_y is not None else None
