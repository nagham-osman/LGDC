...
class Reduction(ABC):
    ...
    def get_reduced_graph(self, rng=np.random.default_rng()):
        if self.latent_graph_size > 0:
            C = self.get_coarsening_matrix_fixed(rng)
        else:
            C = self.get_coarsening_matrix_with_threshold(rng)
    ...
    def get_B0(self) -> NDArray:
        # get_B0 computes a spectral representation (eigenbasis) of the graph Laplacian
    ...
    def get_A(self) -> NDArray:
        # A = B @ (B.T @ L @ B)^-1/2, eigen basis for normalized Laplacian
    ...
    def get_coarsening_matrix_with_threshold(self, rng) -> coo_array:
    ...
    def get_coarsening_matrix_fixed(self, rng) -> coo_array:
        """
        Pick exactly (self.n - latent_graph_size) edge‐contractions
        via randomized‐greedy, then build and return the coarsening matrix P.
        """
        n = self.n
        target_sets = n - self.latent_graph_size
        if target_sets <= 0:
            return coo_array(eye(n), dtype=np.float64)

        C = self.get_contraction_sets()  # (num_edges, 2)
        costs = (
            np.apply_along_axis(self.get_cost, 1, C)
            if len(C) > 0
        ...
        for edge in C:
            if (not marked[edge].any()) and (rng.uniform() >= self.rand_lambda):
                picked.append(edge)
                marked[edge] = True
                if len(picked) >= target_sets:
                    break
    ...
    def get_coarsening_matrix(self, rng) -> coo_array:
        # get the contraction sets and their costs
        # Edge reduction gives a contraction set as a pair of nodes connected by an edge
        # Neighborhood contraction gives one ContSet as node connected to one node
        contraction_sets = self.get_contraction_sets()
        costs = (
            np.apply_along_axis(self.get_cost, 1, contraction_sets)
            if len(contraction_sets) > 0
        ...
        for contraction_set in contraction_sets:
            if (
                ...
                partitions.append(contraction_set)
                marked[contraction_set] = True
                remaining_nodes -= len(contraction_set) - 1
                if remaining_nodes <= self.latent_graph_size:
                    final_round_flag = 1
                    break
        # If the reduced nodes was larger than latent_graph_size
        # Ensure fixed reduced graph size

        # Todo: modify the function to process the case when last node is not laten_graph_size
        """
        if final_round_flag:
            if remaining_nodes < self.latent_graph_size:
                extra_nodes_to_remove = remaining_nodes - self.latent_graph_size
            if extra_nodes_to_remove > 0:
                nodes_to_remove = remaining_unmarked[:extra_nodes_to_remove]
                marked[nodes_to_remove] = True
        """
...
class ReductionFactory:
    ...
    def __call__(self, adj: NDArray) -> Reduction:
        ...
        reduction.latent_graph_size = self.latent_graph_size
