...
class DiscreteGraphDiffusionModel(Module):
    ...
    def forward(
        ...
        default_nc = t.float() / self.num_steps
        noise_cond = model_kwargs.pop("noise_cond", default_nc)
...
class DiscreteGraphDiffusion:
    def __init__(self, X_dim, E_dim, self_conditioning, num_steps, sampling_mode = 'uniform'):
        super().__init__()

        self._device = th.device("cuda" if th.cuda.is_available() else "cpu")
        self.model_wrapper = DiscreteGraphDiffusionModel(
            self_conditioning, X_dim, E_dim, num_steps
        )
        self.X_dim = X_dim
        self.E_dim = E_dim
        self.num_steps = num_steps
        self.node_diffusion = CategoricalDiffusion(X_dim, num_steps)
        self.edge_diffusion = CategoricalDiffusion(E_dim, num_steps)
    ...
    def sample(self, edge_index, batch, model, model_kwargs):
        ...
        """
        # sample from p(x_T)
        node_attr_t = (sample_categorical(self.node_diffusion.qT, batch.size(0))).to(self.device)
        ...
        for timestep in reversed(range(self.num_steps)):
            ...
            if self.sampling_mode == "polydec":
                tau = t.float() / (self.num_steps - 1)
                noise_cond = 2 * tau - tau ** 2
                t_star = (noise_cond * (self.num_steps - 1)).round().long()
                t_eff = t_star.clamp_(0, self.num_steps - 1)
            else:
                t_eff = t
                noise_cond = t.float() / self.num_steps
            ...
            node_attr_t = self.node_diffusion.q_reverse_sample(
                node_attr_t, node_pred, batch, t_eff
            )
    ...
    def get_loss(self, edge_index, batch, node_attr, edge_attr, model, model_kwargs):
        ...
        if self.sampling_mode in ['polydec', 'polyinc', 'cos', 'revcos']:
            tau = t.float() / (self.num_steps - 1)
            if self.sampling_mode == 'polydec':
                noise_cond = 2 * tau - tau ** 2
            elif self.sampling_mode == "polyinc":
                noise_cond = tau ** 2
            elif self.sampling_mode == "cos":
                noise_cond = (1 - th.cos(tau * th.pi)) / 2
            elif self.sampling_mode == "revcos":
                noise_cond = (2 * tau - (1 - th.cos(tau * th.pi)) / 2)
            t_star = (noise_cond * (self.num_steps - 1)).round().long()
            t_eff = t_star.clamp_(0, self.num_steps - 1)
        else:
            t_eff = t
    ...
    def get_aug_pred(self, edge_index, batch, node_attr, edge_attr, model, model_kwargs):
        """Compute loss to train the model.
...
class CategoricalDiffusion(Module):
    ...
    def q_reverse_sample(self, x_t, pred, batch, t):
        ...
        self.Qs = self.Qs.to(x_t.device)
        self.Qbs = self.Qbs.to(x_t.device)
