...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.train_loss = TrainLossDiscrete(self.cfg.model.lambda_train, (not self.cfg.model.uncon_gen))
        ...
        if self.backbone_model == 'graphformer_con':
            ...
            self.model = GraphTransformerUncon(n_layers=cfg.model.n_layers,
                                          ...
                                          act_fn_in=nn.ReLU(),
                                          act_fn_out=nn.ReLU())
        elif self.backbone_model == 'gine':
            from models.gine_dense import DenseGINNet
            self.model = DenseGINNet(n_layers=cfg.model.n_layers,
                                          input_dims=input_dims,
                                          hidden_mlp_dims=cfg.model.hidden_mlp_dims,
                                          hidden_dims=cfg.model.hidden_dims,
                                          output_dims=output_dims,
        ...
        elif self.backbone_model == 'gat':
            from models.gat import DenseGATNet
            self.model = DenseGATNet(n_layers=cfg.model.n_layers,
                                          input_dims=input_dims,
                                          hidden_mlp_dims=cfg.model.hidden_mlp_dims,
                                          hidden_dims=cfg.model.hidden_dims,
                                          output_dims=output_dims,
        ...
        elif self.backbone_model == 'graphformer_lowrank':
            from models.graphformer_lowrank import GraphTransformerLowrank
            self.model = GraphTransformerLowrank(n_layers=cfg.model.n_layers,
                                          input_dims=input_dims,
                                          hidden_mlp_dims=cfg.model.hidden_mlp_dims,
                                          hidden_dims=cfg.model.hidden_dims,
                                          output_dims=output_dims,
                                          act_fn_in=nn.ReLU(),
                                          act_fn_out=nn.ReLU())
        elif self.backbone_model == 'graphformer_bigbird':
            from models.graphformer_bigbird import GraphTransformerBigBird
            self.model = GraphTransformerBigBird(n_layers=cfg.model.n_layers,
                                                 input_dims=input_dims,
                                                 hidden_mlp_dims=cfg.model.hidden_mlp_dims,
                                                 hidden_dims=cfg.model.hidden_dims,
                                                 output_dims=output_dims,
                                                 block_size=cfg.model.blc_size,
                                                 num_random_blocks=cfg.model.rd_blcs,
                                                 num_global_tokens=cfg.model.global_tks,
                                                 act_fn_in=nn.ReLU(),
                                                 act_fn_out=nn.ReLU())
        ...
        if cfg.model.transition == 'uniform':
            ...
            if cfg.model.uncon_gen is False:
                self.transition_model = DiscreteUniformTransition(x_classes=self.Xdim_output,
                ...
                self.transition_model = DiscreteUniformTransition(x_classes=self.Xdim_output,
                                                                  e_classes=self.Edim_output,
                                                                  y_classes=0)
                self.limit_dist = utils.PlaceHolder(X=x_limit, E=e_limit, y=None)
    ...
    def training_step(self, data, i):
        ...
        if self.cfg.reduction.coarsening:
    ...
    def on_fit_start(self) -> None:
        ...
        if self.cfg.model.uncon_gen is False:
            ...
            self.print("Size of the input features", self.Xdim, self.Edim, None)
    ...
    def compute_Lt(self, X, E, y, pred, noisy_data, node_mask, test):
        ...
        pred_probs_y = F.softmax(pred.y, dim=-1) if self.cfg.model.uncon_gen is False else None
    ...
    def reconstruction_logp(self, t, X, E, node_mask):
        ...
        proby0 = F.softmax(pred0.y, dim=-1) if self.cfg.model.uncon_gen is False else None
    ...
    def forward(self, noisy_data, extra_data, node_mask, training=False):
