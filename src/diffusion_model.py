...
class LiftedDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features=None,
        ...
        self.backbone_model = cfg.model.backbone
        ...
        if cfg.model.uncon_gen is False:
        ...
        self.train_loss = TrainLoss((not self.cfg.model.uncon_gen))
        ...
        if self.backbone_model == 'graphformer_con':
            from models.graphformer_con import GraphTransformerCon
            self.model = GraphTransformerCon(n_layers=cfg.model.n_layers,
                                             ...
                                             act_fn_in=nn.ReLU(),
                                             act_fn_out=nn.ReLU())
        elif self.backbone_model == 'graphformer_uncon':
            from models.graphformer_uncon import GraphTransformerUncon
            self.model = GraphTransformerUncon(n_layers=cfg.model.n_layers,
                                               input_dims=input_dims,
                                               hidden_mlp_dims=cfg.model.hidden_mlp_dims,
                                               hidden_dims=cfg.model.hidden_dims,
                                               output_dims=output_dims,
    ...
    def training_step(self, data, i):
        ...
        y = data.y if self.cfg.model.uncon_gen is False else None
    ...
    def validation_step(self, data, i):
        ...
        y = data.y if self.cfg.model.uncon_gen is False else None
        normalized_data = utils.normalize(X, E, y, self.norm_values, self.norm_biases, node_mask)
        noisy_data = self.apply_noise(normalized_data.X, normalized_data.E, y, node_mask)
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.val_counter % self.cfg.general.sample_every_val == 0:
            ...
            self.sampling_metrics(samples, self.name, self.current_epoch, val_counter=-1,
                                  local_rank=self.local_rank, test=False)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self.sampling_metrics.reset()
        self.sampling_metrics(samples, self.name, self.current_epoch, self.val_counter, self.local_rank, test=True)
        self.sampling_metrics.reset()
    ...
    def apply_noise(self, X, E, y, node_mask):
        ...
        # TODO: This is not elegant though.
        y_size = y.size() if self.cfg.model.uncon_gen is False else None
        eps = diffusion_utils.sample_feature_noise(X.size(), E.size(), y_size, node_mask).type_as(X)

        # Sample z_t given x, h for timestep t, from q(z_t | x, h)
        X_t = alpha_t * X + sigma_t * eps.X
        E_t = alpha_t.unsqueeze(1) * E + sigma_t.unsqueeze(1) * eps.E
        y_t = alpha_t.squeeze(1) * y + sigma_t.squeeze(1) * eps.y if self.cfg.model.uncon_gen is False else None
        ...
        '''
        print("Apply noisy data")
        for key, value in noisy_data.items():
            if noisy_data[key] is not None:
                print(key)
                print(noisy_data[key].shape)
            else:
                print(key)
                print("None values")
        '''
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y, node_mask, test=False):
        ...
        delta_log_py = -self.ydim_output * np.log(self.norm_values[2]) if not self.cfg.model.uncon_gen else 0
        ...
        weighted_predy_diffusion = sqrt_SNR_weight.squeeze(1) * pred.y if pred.y is not None else torch.Tensor(0)
        weighted_epsy_diffusion = sqrt_SNR_weight.squeeze(1) * epsy if epsy is not None else torch.Tensor(0)

        # Compute the MSE summed over channels
        if test:
            if self.cfg.model.uncon_gen:
        ...
        # TODO: not elegant though.
        y_t_size = y_t.size() if not self.cfg.model.uncon_gen else None
        eps0 = diffusion_utils.sample_feature_noise(X_t.size(), E_t.size(), y_t_size, node_mask).type_as(X_t)

        X_0 = alpha_0 * X_t + sigma_0 * eps0.X
        E_0 = alpha_0.unsqueeze(1) * E_t + sigma_0.unsqueeze(1) * eps0.E
        y_0 = alpha_0.squeeze(1) * y_t + sigma_0.squeeze(1) * eps0.y if not self.cfg.model.uncon_gen else None
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, save_final: int, number_chain_steps: int,
        ...
        y_size = (batch_size, self.ydim_output) if not self.cfg.model.uncon_gen else None
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            y_keep_chain = y[:keep_chain] if y is not None else None
            unnormalized = utils.unnormalize(X=X[:keep_chain], E=E[:keep_chain], y=y_keep_chain,
    ...
    def sample_discrete_graph_given_z0(self, X_0, E_0, y_0, node_mask):
        ...
        noisy_data = {'X_t': X_0, 'E_t': E_0, 'y_t': y_0, 't': torch.zeros(X_0.shape[0], 1).type_as(X_0)}
        extra_data = self.compute_extra_data(noisy_data)
        ...
        pred_y = 1. / alpha_0.squeeze(1) * (y_0 - sigma_0.squeeze(1) * eps0.y) \
            if self.cfg.model.uncon_gen is False else None
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        mu_E = E_t / alpha_t_given_s.unsqueeze(1) \
               - (sigma2_t_given_s / (alpha_t_given_s * sigma_t)).unsqueeze(1) * eps.E
        if not self.cfg.model.uncon_gen:
            mu_y = y_t / alpha_t_given_s.squeeze(1) \
