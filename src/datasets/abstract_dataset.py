...
class AbstractDataModule(LightningDataset):
    def __init__(self, cfg, datasets, collate_fn):
    ...
    def node_counts(self, max_nodes_possible=300):
        ...
        if self.cfg.reduction.coarsening:
            all_counts_c = torch.zeros(max_nodes_possible)
        for loader in [self.train_dataloader(), self.val_dataloader()]:
            for data in loader:
                ...
                for count in counts:
                    all_counts[count] += 1
                if self.cfg.reduction.coarsening:
                    unique_c, counts_c = torch.unique(data.batch_reduced, return_counts=True)
                    for count in counts_c:
                        all_counts_c[count] += 1

        if self.cfg.reduction.coarsening:
            max_index_c = max(all_counts_c.nonzero())
            all_counts_c = all_counts_c[:max_index_c + 1]
            all_counts_c = all_counts_c / all_counts_c.sum()
            return all_counts, all_counts_c
